...
## For suan
- crawling資料夾內的程式會使用到我們insertion的API，可以作為一種測試
- MongoDB是我們主要要測試的目標(檔案在root裡面了)
- 關於DB的schema可以參考db/README.md
- test_Database相關檔案放在DB資料夾中 (這個寫好了，只是可以當參考範例)

DB連線方法
1. 上網抓docker image "mongo"，版本mongo:7.0-rc (latest也可以)
2. 跑`docker run --publish 27017:27017 --name my-mongodb -d mongo`
3. 在終端機打`ifconfig`，看一下自己連到docker的ip是多少(wsl通常是eth0)
4. 到`dbAPI/db_config.py`裡面把IP改成自己的IP
這樣就能跑了，確認方法是跑API_example沒卡住

## For Liu
- 因為MongoDB.py被拖到root了，在有使用到MongoDB class的檔案import路徑都是`from MongoDB import MongoDB`，但是在推回去的時候要注意改回`from dbAPI.MongoDB import MongoDB`
